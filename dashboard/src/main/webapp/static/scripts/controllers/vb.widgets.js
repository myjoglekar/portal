...
//        }, 100);
//    };
//    $scope.addWidget = function (newWidget) { alert()      //Add Widget
//        var data = {
...
app.directive('dynamicTable', function ($http, $filter, $stateParams, $sce) {
    return{
        ...
        template: '<div ng-show="loadingTable" class="text-center" style="color: #228995;"><img src="static/img/logos/loader.gif"></div>' +
                '<table ng-if="ajaxLoadingCompleted" class="table table-responsive table-bordered table-striped table-l2t" ng-hide="hideEmptyTable">' +
                '<thead><tr>' +
                '<th class="table-bg" ng-if="groupingName" class="text-center">' +
                ...
                ' Group</th>' +
                '<th class="text-capitalize table-bg" ng-repeat="col in columns" ng-if="col.columnHide == null">' +
                ...
                '<tbody class="grouping-bg" ng-repeat="grouping in groupingData.data">' +
                '<tr ng-if="!isZeroRow(grouping, columns)" class="text-capitalize group-bg">' +
                ...
                '</td>' +
                '</tr>' +
                '<tr ng-if="!isZeroRow(item, columns)" ng-show="grouping.$hideRows" ng-repeat-start="item in grouping.data" class="text-capitaliz">' +
...
//                $http.get("admin/proxy/getJson?url=" + scope.lineChartUrl + "&widgetId=" + scope.widgetId + "&startDate=" + $stateParams.startDate + "&endDate=" + $stateParams.endDate + "&dealerId=" + $stateParams.dealerId + "&dateDuration=" + scope.widgetDateDuration + "&frequencyDuration=" + scope.widgetFrequencyDuration + "&customRange=" + scope.customRange).success(function (response) {
                    ...
                    if (response.data.length === 0) {
                        ...
                        var chart = c3.generate({
                            ...
                            color: {
                                pattern: ['#71C7A7', '#E58C00', '#F0CC5A', '#009B96', '#236570', '#9F4462', '#E15E64']
                            },
                            tooltip: {show: false},
                            axis: {
                                x: {
...
//                $http.get("admin/proxy/getJson?url=" + scope.barChartUrl + "&widgetId=" + scope.widgetId + "&startDate=" + $stateParams.startDate + "&endDate=" + $stateParams.endDate + "&dealerId=" + $stateParams.dealerId + "&dateDuration=" + scope.widgetDateDuration + "&frequencyDuration=" + scope.widgetFrequencyDuration + "&customRange=" + scope.customRange).success(function (response) {
                    ...
                    if (response.data.length === 0) {
                        ...
                        var chart = c3.generate({
                            ...
                            color: {
                                pattern: ['#E58C00', '#71C7A7', '#F0CC5A', '#009B96', '#236570', '#9F4462', '#E15E64']
                            },
                            tooltip: {show: false},
                            axis: {
                                rotated: true,
